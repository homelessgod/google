...
ExtensionLogic = function() {
  ...
  this.Policies = {
    ...
    BROWSER_SWITCHER_ENABLED: 'browser_switcher_enabled'
  };
 ...
 * @param {string} prefix The prefix of the url that has been omitted from the
 *                 entry.
 */
ExtensionLogic.prototype.isProperPrefix = function(prefix) {
  return (prefix === "https://") || (prefix === "https:") ||
...
ExtensionLogic.prototype.urlListForceRedirection = function(url, list) {
  ...
  list.forEach(function(item) {
    switch (item.type) {
      case self.UrlPatternType.WILDCARD:
        if (reason.length < item.url.length) {
          reason = item.url;
          transition = true;
        }
        break;
      case self.UrlPatternType.HOST:
        ...
        break;
      case self.UrlPatternType.NEGATED_HOST:
        if (anchor.hostname.indexOf(item.url) !== -1) {
          if (reason.length < item.url.length) {
            reason = item.url;
            transition = false;
        ...
        break;
      case self.UrlPatternType.PREFIX:
        pos = url.indexOf(item.url)
        if (pos === 0 ||
            (pos !== -1 && self.isProperPrefix(url.substr(0, pos)))) {
          if (reason.length < item.url.length) {
            reason = item.url;
            transition = true;
          }
        ...
        break;
      case self.UrlPatternType.NEGATED_PREFIX:
        pos = url.indexOf(item.url)
        if (pos === 0 ||
            (pos !== -1 && self.isProperPrefix(url.substr(0, pos)))) {
          if (reason.length < item.url.length) {
...
ExtensionLogic.prototype.urlGreyListForceStaying = function(url) {
  ...
  var reason = '';
  var transition = false;
  this.url_greylist_info.forEach(function(item) {
    switch (item.type) {
      case self.UrlPatternType.HOST:
        if (anchor.hostname.indexOf(item.url) !== -1) {
          if (reason.length < item.url.length) {
...
ExtensionLogic.prototype.normalizeUrl = function(item) {
  if (item.indexOf('/') === -1 || item === '*') {
    ...
    var normalized_url = anchor.href;
    // When the input URL has no schema the anchor element will add the current
    // schema to it which is the extensions schema or even base url if no // is
    // present so we have to clean it back out.
    var extension_origin = chrome.extension.getURL('');
    if (normalized_url.indexOf(extension_origin) == 0)
      normalized_url = normalized_url.substring(extension_origin.length)
    else if (normalized_url.indexOf('chrome-extension://') == 0)
      normalized_url = normalized_url.substring(19)
...
ExtensionLogic.prototype.createRules = function(items) {
  ...
  // If the new Chrome policy to turn on the built-in feature is on. Clear the
  // extension state and thus incapacitate it.
  if (items[this.Policies.BROWSER_SWITCHER_ENABLED]) {
    self.url_list_info = [];
    self.url_greylist_info = [];
    self.ie_site_list_info = [];
    self.url_list_loaded = true;
    // With this here the extension will need a restart to work again but there-
    // fore will consume way less resources when it is not operational than when
    // the listener is kept.
    chrome.webRequest.onBeforeRequest.removeListener(this.request_listener);
    this.port.logError(
        "Disabling extension because LBS Feature in Chrome is enabled.");
    return;
  }
...
ExtensionLogic.prototype.updateConfiguration = function(change, area) {
  ...
  if (area === 'managed') {
    chrome.storage.managed.get([this.Policies.URL_LIST,
                                ...
                                this.Policies.BROWSER_SWITCHER_ENABLED],
